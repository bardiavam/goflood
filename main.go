...
func main() {
        ...
        // Proxy Grabber Parameters
        grabProxiesFlag := flag.Bool("grabproxies", false, "Grab proxies from public sources and save to file before starting")
        proxyGrabOutputFlag := flag.String("graboutput", "proxies.txt", "Output file for grabbed proxies")
        ...
        // Handle proxy grabbing if requested
        if *grabProxiesFlag {
            log.Printf("Proxy grabbing requested. Will save proxies to %s", *proxyGrabOutputFlag)
            count, err := GrabProxies(*proxyGrabOutputFlag, *proxyCheckTargetFlag, *verboseFlag)
            if err != nil {
                log.Fatalf("Error grabbing proxies: %v", err)
            }
            if count == 0 {
                log.Fatal("Error: No working proxies found. Cannot proceed.")
            }
            // Use the grabbed proxies file
            *proxyFileFlag = *proxyGrabOutputFlag
            log.Printf("Successfully grabbed %d proxies. Will use file: %s", count, *proxyFileFlag)
        }
        
        // Now check if we have a proxy file (either specified or created by grabber)
        if *proxyFileFlag == "" {
                log.Fatal("Error: Proxy file path (-proxies) is required or use -grabproxies to fetch them automatically.")
        }
